...
- `app/Core`: capability lintas-domain. Isinya sepuluh: `Organisasi` (tenancy),
  `Izin`, `Audit`, `Penomoran`, `Konfigurasi`, `Host`, `Entitas`, `Idempotensi`,
  `Peristiwa`, `Cadangan`.
- `app/Domain/<Domain>`: bounded context bisnis. Berkas, notifikasi, persetujuan,
  integrasi, dan sinkronisasi tinggal di sini sebagai domain, bukan di `app/Core`.
