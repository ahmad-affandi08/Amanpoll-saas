...
interface LokasiRingkas { Id: string; Nama: string }
...
function DialogFormGudang({ gudang, lokasi }: { gudang: Gudang | null; lokasi: LokasiRingkas[] }) {
  ...
  const submit = (e: FormEvent) => {
    ...
    const opsi = { onSuccess: () => { setBuka(false); if (!gudang) form.reset(); } };
    if (gudang) {
      router.put(`/gudang/${gudang.Id}`, payload, opsi);
    } else {
      router.post('/gudang', payload, opsi);
    }
  };
  ...
  return (
    <Dialog open={buka} onOpenChange={setBuka}>
      <DialogTrigger asChild>
        <Button variant={gudang ? 'outline' : 'default'} size={gudang ? 'sm' : 'default'}>{gudang ? 'Ubah' : 'Tambah Gudang'}</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{gudang ? 'Ubah Gudang' : 'Tambah Gudang'}</DialogTitle></DialogHeader>
...
function DialogLokasiGudang({ gudang, lokasiGudang }: { gudang: Gudang; lokasiGudang: LokasiGudang[] }) {
  ...
  const tambah = (e: FormEvent) => {
    e.preventDefault();
    router.post(`/gudang/${gudang.Id}/lokasi`, { ...form.data, IndukId: form.data.IndukId === TANPA ? null : form.data.IndukId }, {
  ...
  return (
    <Dialog open={buka} onOpenChange={setBuka}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm">Lokasi ({lokasiGudang.length})</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader><DialogTitle>Lokasi dalam {gudang.Nama}</DialogTitle></DialogHeader>
        <form onSubmit={tambah} className="flex items-end gap-2">
          ...
          <Button type="submit" size="sm" disabled={form.processing}>Tambah</Button>
        </form>
        <div className="space-y-2">
          ...
          {lokasiGudang.map((l) => (
            <div key={l.Id} className="flex items-center justify-between rounded-md border border-border px-3 py-2 text-sm">
              <div>
                ...
                {l.NamaInduk && <span className="ml-2 text-xs text-muted-foreground">di dalam {l.NamaInduk}</span>}
              </div>
              <Button variant="ghost" size="sm" onClick={() => hapus(l)}>Hapus</Button>
            </div>
          ))}
        </div>
      </DialogContent>
...
export default function GudangIndex({ gudang, lokasiGudangPerGudang, lokasi }: Props) {
  const hapus = (item: Gudang) => {
    ...
    router.delete(`/gudang/${item.Id}`, { preserveScroll: true });
  };
  ...
  const columns = useMemo<ColumnDef<Gudang>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => <Badge variant={VARIAN_BADGE_STATUS_GUDANG[row.original.Status]}>{row.original.Status}</Badge>,
      meta: { label: 'Status' },
    },
    {
      id: 'aksi',
      ...
      cell: ({ row }) => (
        <div className="flex justify-end gap-2">
          <DialogLokasiGudang gudang={row.original} lokasiGudang={lokasiGudangPerGudang[row.original.Id] ?? []} />
    ...
    <AppLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Kelola gudang beserta lokasi penyimpanan di dalamnya.</p>
        </div>
        <DialogFormGudang gudang={null} lokasi={lokasi} />
      </div>

      {gudang.length === 0 ? (
        <EmptyState ilustrasi="/assets/3d/gudang.webp" judul="Belum ada gudang." deskripsi="Tambahkan gudang pertama untuk mulai mencatat stok." />
      ) : (
        <DataTable columns={columns} data={gudang} pencarianPlaceholder="Cari nama atau kode gudang..." pesanKosong="Belum ada gudang." />
      )}
    </AppLayout>
  );
}
