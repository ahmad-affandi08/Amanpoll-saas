...
function DialogFormAlur({ alur, jenisEntitasTersedia }: { alur: AlurPersetujuan | null; jenisEntitasTersedia: string[] }) {
  const [buka, setBuka] = useState(false);
  const form = useForm({
    Kode: alur?.Kode ?? '', Nama: alur?.Nama ?? '', JenisEntitas: alur?.JenisEntitas ?? (jenisEntitasTersedia[0] ?? ''),
  });

  const submit = (e: FormEvent) => {
    e.preventDefault();
    const opsi = { onSuccess: () => { setBuka(false); if (!alur) form.reset(); } };
    if (alur) {
      router.put(`/persetujuan/alur/${alur.Id}`, form.data, opsi);
    } else {
      router.post('/persetujuan/alur', form.data, opsi);
    }
  };
  ...
  return (
    <Dialog open={buka} onOpenChange={setBuka}>
      <DialogTrigger asChild>
        <Button variant={alur ? 'outline' : 'default'} size={alur ? 'sm' : 'default'}>{alur ? 'Ubah' : 'Tambah Alur'}</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{alur ? 'Ubah Alur Persetujuan' : 'Tambah Alur Persetujuan'}</DialogTitle></DialogHeader>
...
function FormTahap({ alurPersetujuanId, tahap, peran, pengguna, onSelesai }: {
  alurPersetujuanId: string; tahap: TahapPersetujuan | null; peran: Peran[]; pengguna: Pengguna[]; onSelesai: () => void;
}) {
  const form = useForm({
  ...
  const submit = (e: FormEvent) => {
    ...
    const opsi = { onSuccess: () => { onSelesai(); if (!tahap) form.reset(); } };
    if (tahap) {
      router.put(`/persetujuan/tahap/${tahap.Id}`, payload, opsi);
    } else {
      router.post(`/persetujuan/alur/${alurPersetujuanId}/tahap`, payload, opsi);
    }
  };
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-2 gap-3 rounded-md border border-border p-3">
      <div className="space-y-1.5">
        <Label>Urutan</Label>
        <Input type="number" min={1} value={form.data.Urutan} onChange={(e) => form.setData('Urutan', Number(e.target.value))} />
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <Select value={form.data.JenisPenyetuju} onValueChange={(v) => form.setData('JenisPenyetuju', v as JenisPenyetuju)}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            {JENIS_PENYETUJU.map((j) => <SelectItem key={j} value={j}>{j}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      {form.data.JenisPenyetuju === 'Pengguna' && (
        <div className="space-y-1.5">
          ...
          <Select value={form.data.PenggunaId || TANPA} onValueChange={(v) => form.setData('PenggunaId', v === TANPA ? '' : v)}>
            <SelectTrigger><SelectValue placeholder="Pilih pengguna" /></SelectTrigger>
            <SelectContent>
              {pengguna.map((p) => <SelectItem key={p.Id} value={p.Id}>{p.Nama}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      )}
      {(form.data.JenisPenyetuju === 'Peran' || form.data.JenisPenyetuju === 'Unit') && (
        <div className="space-y-1.5">
          ...
          <Select value={form.data.PeranId || TANPA} onValueChange={(v) => form.setData('PeranId', v === TANPA ? '' : v)}>
            ...
            <SelectContent>
              ...
              {peran.map((p) => <SelectItem key={p.Id} value={p.Id}>{p.Nama}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      )}
      <div className="space-y-1.5">
        ...
        <Input type="number" min={1} value={form.data.JumlahMinimumPenyetuju} onChange={(e) => form.setData('JumlahMinimumPenyetuju', Number(e.target.value))} />
      </div>
      <label className="flex items-center gap-2 self-end pb-2 text-sm">
        <Checkbox checked={form.data.BolehMenyetujuiSendiri} onCheckedChange={(v) => form.setData('BolehMenyetujuiSendiri', Boolean(v))} />
        Boleh menyetujui permintaan sendiri
      </label>
      <div className="col-span-2 flex justify-end">
        <Button type="submit" size="sm" disabled={form.processing}>{tahap ? 'Simpan Perubahan' : 'Tambah Tahap'}</Button>
      </div>
    </form>
  );
}

function DialogKelolaTahap({ alur, peran, pengguna }: { alur: AlurPersetujuan; peran: Peran[]; pengguna: Pengguna[] }) {
  const [buka, setBuka] = useState(false);
  ...
  return (
    <Dialog open={buka} onOpenChange={setBuka}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">Kelola Tahap</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader><DialogTitle>Tahap Persetujuan -- {alur.Nama}</DialogTitle></DialogHeader>
        {alur.Aktif && (
        ...
        <div className="space-y-2">
          {alur.TahapPersetujuan.map((tahap) => (
            <div key={tahap.Id} className="flex items-center justify-between rounded-md border border-border px-3 py-2">
              <div className="text-sm">
                <span className="font-medium text-foreground">{tahap.Urutan}. {tahap.Nama}</span>
                <span className="ml-2 text-muted-foreground">
                  ({tahap.JenisPenyetuju}{tahap.NamaPengguna ? `: ${tahap.NamaPengguna}` : ''}{tahap.NamaPeran ? `: ${tahap.NamaPeran}` : ''}, min. {tahap.JumlahMinimumPenyetuju})
                </span>
              </div>
              {!alur.Aktif && (
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={() => setMengedit(tahap)}>Ubah</Button>
                  <Button variant="ghost" size="sm" onClick={() => hapus(tahap)}>Hapus</Button>
                </div>
              )}
            </div>
          ))}
          {alur.TahapPersetujuan.length === 0 && <p className="text-sm text-muted-foreground">Belum ada tahap.</p>}
        </div>
        {!alur.Aktif && (
          <div className="border-t border-border pt-4">
            <p className="mb-2 text-sm font-medium text-foreground">{mengedit ? `Ubah Tahap ${mengedit.Urutan}` : 'Tambah Tahap Baru'}</p>
            <FormTahap
...
export default function AlurPersetujuanIndex({ alurPersetujuan, jenisEntitasTersedia, peran, pengguna }: Props) {
  const hapus = (alur: AlurPersetujuan) => {
    if (!confirm(`Hapus alur "${alur.Nama}"?`)) return;
    router.delete(`/persetujuan/alur/${alur.Id}`, { preserveScroll: true });
  };

  const toggleAktif = (alur: AlurPersetujuan) => {
    const url = alur.Aktif ? `/persetujuan/alur/${alur.Id}/nonaktifkan` : `/persetujuan/alur/${alur.Id}/aktifkan`;
    ...
    <AppLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Definisikan tahapan persetujuan untuk berbagai jenis entitas.</p>
        </div>
