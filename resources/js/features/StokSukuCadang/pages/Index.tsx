...
export default function StokSukuCadangIndex({ stok, gudang, sukuCadang, filter }: Props) {
  ...
  const columns = useMemo<ColumnDef<StokSukuCadang>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          {row.original.NamaLokasiGudang && <div className="text-xs text-muted-foreground">{row.original.NamaLokasiGudang}</div>}
        </div>
      ),
      meta: { label: 'Gudang' },
    },
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="font-semibold text-foreground">{row.original.JumlahTersediaBersih}</span>,
  ...
  return (
    <AppLayout>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Saldo stok per gudang -- hanya baca. Perubahan hanya lewat Mutasi Stok atau Reservasi.</p>
      </div>

      <div className="mb-4 grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label>Gudang</Label>
          <Select value={gudangId} onValueChange={(v) => { setGudangId(v); terapkanFilter(v, sukuCadangId); }}>
        ...
        <div className="space-y-1.5">
          ...
          <Select value={sukuCadangId} onValueChange={(v) => { setSukuCadangId(v); terapkanFilter(gudangId, v); }}>
            <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      {stok.length === 0 ? (
        <EmptyState ilustrasi="/assets/3d/suku-cadang.webp" judul="Belum ada saldo stok." deskripsi="Saldo stok akan muncul setelah mutasi stok pertama diposting." />
      ) : (
        <DataTable columns={columns} data={stok} pencarianPlaceholder="Cari suku cadang atau gudang..." pesanKosong="Tidak ada saldo stok yang cocok." />
      )}
    </AppLayout>
  );
}
