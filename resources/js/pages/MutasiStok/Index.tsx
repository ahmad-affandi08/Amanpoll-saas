...
const LABEL_JENIS: Record<JenisMutasiStok, string> = {
  Penerimaan: 'Penerimaan', Pengeluaran: 'Pengeluaran', Transfer: 'Transfer', Adjustment: 'Penyesuaian', Return: 'Retur',
};
...
function DialogBuatMutasi({ gudang }: { gudang: Ringkas[] }) {
  ...
  const form = useForm({ Jenis: 'Penerimaan' as JenisMutasiStok, GudangAsalId: TANPA, GudangTujuanId: TANPA, Catatan: '' });

  const submit = (e: FormEvent) => {
    e.preventDefault();
    router.post('/mutasi-stok', {
  ...
  return (
    <Dialog open={buka} onOpenChange={setBuka}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle>Buat Mutasi Stok</DialogTitle></DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          <div className="space-y-1.5">
            <Label>Jenis</Label>
            <Select value={form.data.Jenis} onValueChange={(v) => form.setData('Jenis', v as JenisMutasiStok)}>
              <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
              <SelectContent>
                {(Object.keys(LABEL_JENIS) as JenisMutasiStok[]).map((j) => <SelectItem key={j} value={j}>{LABEL_JENIS[j]}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          ...
          {butuhGudangTujuan(form.data.Jenis) && (
            <div className="space-y-1.5">
              ...
              <Select value={form.data.GudangTujuanId} onValueChange={(v) => form.setData('GudangTujuanId', v)}>
                <SelectTrigger className="w-full"><SelectValue placeholder="Pilih gudang" /></SelectTrigger>
                <SelectContent>
                  {gudang.map((g) => <SelectItem key={g.Id} value={g.Id}>{g.Nama}</SelectItem>)}
                </SelectContent>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea value={form.data.Catatan} onChange={(e) => form.setData('Catatan', e.target.value)} rows={3} />
    ...
    <AppLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Penerimaan, pengeluaran, transfer, penyesuaian, dan retur -- draf, posting, sampai audit.</p>
        </div>
        <DialogBuatMutasi gudang={gudang} />
      </div>

      {mutasiStok.length === 0 ? (
        <EmptyState ilustrasi="/assets/3d/persediaan.webp" judul="Belum ada mutasi stok." deskripsi="Buat mutasi pertama untuk mulai mencatat pergerakan stok." />
      ) : (
        <div className="space-y-2">
          {mutasiStok.map((m) => (
            <Link key={m.Id} href={`/mutasi-stok/${m.Id}`} className="flex items-center justify-between rounded-[9px] border border-border bg-card p-4 hover:border-teknisi-600/40">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{new Date(m.Tanggal).toLocaleString('id-ID')} &middot; {m.NamaDibuatOleh}</div>
              </div>
