...
export function KomentarTab({ jenisEntitas, entitasId }: Props) {
  ...
  const muat = () => {
    setMemuat(true);
    http
      .get(ruteKolaborasi.komentar, { params: { jenisEntitas, entitasId } })
  ...
  const kirim = (e: FormEvent) => {
    ...
    router.post(
      ruteKolaborasi.komentar,
      { JenisEntitas: jenisEntitas, EntitasId: entitasId, Isi: isiBaru },
      {
        preserveScroll: true,
        onSuccess: () => {
          setIsiBaru('');
  ...
  const simpanEdit = () => {
    if (!mengedit) return;
    router.put(
      ruteKolaborasi.komentarDetail(mengedit.id),
      { Isi: mengedit.isi },
      {
        preserveScroll: true,
        onSuccess: () => {
    ...
    <div className="space-y-3">
      <form onSubmit={kirim} className="space-y-2">
        ...
        <div className="flex justify-end">
          <Button type="submit" size="sm" disabled={!isiBaru.trim()}>
            Kirim
          </Button>
        </div>
      </form>
            ...
            <div key={item.Id} className="rounded-md border border-border p-3">
              ...
              {mengedit?.id === item.Id ? (
                <div className="space-y-2">
                  <Textarea
                    value={mengedit.isi}
                    onChange={(e) => setMengedit({ id: item.Id, isi: e.target.value })}
                    rows={2}
                  />
                  <div className="flex justify-end gap-2">
                    <Button size="sm" variant="outline" onClick={() => setMengedit(null)}>
                      Batal
                    </Button>
                    <Button size="sm" onClick={simpanEdit}>
                      Simpan
                    </Button>
                  </div>
                </div>
                  ...
                  <div className="mt-1 flex justify-end gap-3">
                    {penulis && (
                      <button
                        ...
                        onClick={() => setMengedit({ id: item.Id, isi: item.Isi })}
                      >
                        Ubah
                      </button>
                    )}
